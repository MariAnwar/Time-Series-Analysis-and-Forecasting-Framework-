...
from Model_selection import Model_Selection  # import your wrapper
import os
    ...
    with st.container():
        ...
        if st.button(f"⚡ Run {st.session_state.selected_model}", type="primary"):
            ...
            if st.session_state.selected_model == "TACTIS-2":
                ...
                fig = go.Figure()
                actual = data_results["actual"][4]
                fig.add_trace(go.Scatter(y=actual, mode="lines", name="Actual", line=dict(color="blue")))

                for forecast in data_results["forecasts"]:
                    mean = [m[4] for m in forecast["mean"]]
                    q05 = [q[4] for q in forecast["q05"]]
                    q95 = [q[4] for q in forecast["q95"]]
                    q25 = [q[4] for q in forecast["q25"]]
                    q75 = [q[4] for q in forecast["q75"]]
